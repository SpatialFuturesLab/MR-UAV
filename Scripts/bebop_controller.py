...
# TODO: Implement State Machine, taking Joystick inputs. Do not use keyboard for takeoff, land, etc.
...
# class CapturePoints:
# 	def __init__(self, captureBebop, captureGeom):
...
class States:
	def __init__(self, curr):
		...
		self.stateTransition = {("IDLE", "TAKE_OFF"):"HOVER", ("IDLE", "LAND"):"IDLE", ("IDLE", "NAVIGATE"):"IDLE", ("HOVER", "LAND"):"IDLE", ("HOVER", "TAKE_OFF"):"HOVER", ("HOVER", "NAVIGATE"):"NAVIGATING", ("NAVIGATING", "NAVIGATE"):"NAVIGATING", ("NAVIGATING", "LAND"):"IDLE", ("NAVIGATING", "TAKE_OFF"):"NAVIGATING"}
...
class PositionController:
	def __init__(self):
        ...
        self.threadReader = threading.Thread(target=self.key_reader)
		...
		self.isKeyreaderFinished = False
	...
	def key_reader():
    	with NonBlockingConsole() as nbc:
            ...
            if c == '\x1b':
                self.toggleAutonomous = not self.toggleAutonomous
                print "Autonomous: ", self.toggleAutonomous
                if not self.toggleAutonomous:
                	self.quad.get_next_state("TOGGLE")
                
            # if 'l' is pressed send a landing command to bebop
            elif c == 'l' and self.enableAutonomous:
                self.quad.get_next_state("LAND")
                self.publishLanding.publish(message_empty)
                print "Land"
                rospy.sleep(10)
            # if 'n' is pressed change the state to navigation mode
            elif c == 'n' and self.enableAutonomous:
                self.quad.get_next_state("NAVIGATE")
            # if 't' is pressed send a command to bebop to take off
            elif c == 't' and self.enableAutonomous:
            	self.quad.get_next_state("TAKE_OFF")
                self.publishTakeoff.publish(message_empty)
                print "Take off"
                rospy.sleep(10)
...
if __name__ == '__main__':
    try:
        ...
        position_control_thread = threading.Thread(target=P.move_quad())
        position_control_thread.start()
        C = CapturePoints(True, True)
        rospy.spin()
