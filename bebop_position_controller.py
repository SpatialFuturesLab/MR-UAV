    ...
    while not rospy.is_shutdown() and not is_keyreader_finished:
        ...
        if current_state == state_manager.COPTER_STATE_NAVIGATING:
            ...
            if _set_pt[0]:
                ...
                set_pt = points[current_index]
            current_x = odom_current.pose.position.x
            current_y = odom_current.pose.position.y
            current_z = odom_current.pose.position.z
            bebop_yaw = get_yaw_from_quaternion(odom_current.pose.orientation)
            if bebop_yaw > math.pi:
                bebop_yaw = 2*math.pi - bebop_yaw

            error_pr_x = set_pt[0] - current_x
            error_pr_y = set_pt[1] - current_y
            error_z = set_pt[2] - current_z
            error_yaw = 0 - bebop_yaw

            error_x = math.cos(bebop_yaw) * error_pr_x + math.sin(bebop_yaw) * error_pr_y
            error_y = -math.sin(bebop_yaw) * error_pr_x + math.cos(bebop_yaw) * error_pr_y
            # error_x = error_pr_x
            # error_y = error_pr_y
            #calculates the length of error vector (target pose - current pose)
            length = math.sqrt(error_x **2 + error_y **2 + error_z **2)

            speed_x = pid('x',error_x,0.08,0.00,0.05,2)
            speed_y = pid('y',error_y,0.08,0.00,0.05,2)
            speed_z = pid('z',error_z,0.08,0.00,0.05,2)
            ...
            if odomUpdated(time_current,1) == True:
                twist_current = Twist(Vector3(speed_x,speed_y,speed_z),Vector3(0.0, 0.0, 0.0))
...
if __name__ == '__main__':
    try:
        ...
        rospy.Subscriber("/vrpn_client_node/bebop/pose", PoseStamped, odometry_callback)
